...
class PacienteView(viewsets.ModelViewSet):
    ...
    permission_strategy = PacientePermission()
    paciente_strategy = PacienteStrategy()
    profissional_strategy = ProfissionalStrategy()
    ...
    def partial_update(self, request, *args, **kwargs):
        try:
            self.paciente_strategy.validar_usuario(request)

            if 'username' in request.data:
                item = get_object_or_404(Paciente, user__username=request.data.get("username"))
